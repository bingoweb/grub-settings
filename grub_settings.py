...
class GrubPaths:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Dağıtım: {self.distro_info.get('ID', 'unknown')} ({self.distro_info.get('PRETTY_NAME', 'Unknown Linux')})"
        )
    ...
    def update_windows_ui(self):
        ...
        if self.windows_detected:
            if self.windows_in_grub:
                ...
                self.windows_action_btn.add_css_class("suggested-action")
                self.windows_info_row.set_subtitle(
                    "Windows'u GRUB menüsüne eklemek için butona tıklayın"
                )
        else:
            self.windows_status_row.set_title("❌ Windows Bulunamadı")
            self.windows_status_row.set_subtitle(
                f"{PATHS.efi_path} bölümünde Windows boot dosyası yok"
            )
...
"""
    ...
    def remove_windows_from_grub(self):
        ...
        confirm.set_body(
            "Windows Boot Manager GRUB menüsünden kaldırılacak.\n\nWindows yine de UEFI menüsünden başlatılabilir."
        )
        confirm.add_response("cancel", "İptal")
...
class GrubSettingsApp(Adw.Application):
    ...
    def on_about(self, button):
        about = Adw.AboutWindow(
            ...
            # App Description
            comments=_(
                """A modern, user-friendly GRUB bootloader configuration tool.
...
Built with GTK4 and Libadwaita for a native Linux experience."""
            ...
            # Credits
            developers=["Taylan Soylu <taylansoylu@gmail.com>"],
            designers=["Taylan Soylu"],
            # Copyright
    ...
    def on_command_finished(self, return_code):
        ...
        if return_code == 0:
            ...
            self.status_label.set_label(
                "GRUB başarıyla güncellendi! Pencere 3 saniye içinde kapanacak..."
            )
            self.has_changes = False
            self.apply_btn.set_sensitive(False)
    ...
    def on_custom_command_finished(self, return_code):
        ...
        if success:
            ...
            self.status_icon.set_label("✅")
            self.status_label.set_label(
                "Başarıyla tamamlandı! Pencere 3 saniye içinde kapanacak..."
            )
